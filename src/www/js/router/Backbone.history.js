...
define([
    ...
    if(Backbone.history){
        ...
        //登陆验证器
        var loginValidate = Backbone.history.__loginValidate = {
            _loginFiltersHandles : [],
            _loginFilter : function(route,callback){
                this._loginFiltersHandles.unshift({route:route,callback:callback});
            },

            _checkFilter : function(fragment){
                fragment = Backbone.history.getFragment(fragment || '');

                var b = false;
                var self = this;
                var matched = _.any(this._loginFiltersHandles, function(handler) {
                    if (handler.route.test(fragment)) {
                        if(!self._checkLogin()){
                            handler.callback(fragment);
                            _h._returnUrl = fragment;
                            b = true;
                            return true;
                        }
                    }
        ...
        /**如果已经登陆，不应回到这几个路由中**/
		var exclude = ['mz/user/login','mz/user/register'];
